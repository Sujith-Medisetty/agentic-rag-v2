...
def get_frontend_ui_quality_section() -> str:
    ...
    return "\n".join([
        ...
        "- **Static-only stack — NO backend processes.** The deployment "
        "pipeline serves files from `dist/` statically; there is no way "
        "to run a Flask/Express/FastAPI/Django/Node-server backend per "
        "app. Use Vite + React (scaffolded as above) that builds to "
        "`dist/index.html` + `dist/assets/*`. For persistence use "
        "IndexedDB / localStorage on the device, NOT server-side "
        "SQLite/Postgres. For external data, call public APIs directly "
        "from the browser (with CORS). If the user asks for something "
        "that genuinely needs a backend (auth, shared multi-user data, "
        "secret API keys), REFUSE-AND-EXPLAIN per the rules below — "
        "never silently switch to Flask, Express, or any server-side "
        "language. Building a Python/Node backend produces an app that "
        "CAN'T be deployed via Ojas's Deploy button.",
        ...
        "opens a dialog with a Slug field AND a **Sub-app folder** field. "
        "The Sub-app folder is the directory you built in — for example, "
        "if you scaffolded the Vite app in `my-app/` and `npm run build` "
        "produced `my-app/dist/index.html`, the Sub-app folder is "
        "`my-app`. Empty/blank deploys the session root's `dist/`, which "
        "only works if the build was actually at the session root. The "
        "dialog copy makes this clear. ALWAYS state the Sub-app folder "
        "value in your end-of-turn summary so the user knows what to type: "
        "'Build complete at `<subdir>/dist/`. Open 🚀 Deploy, choose a "
        "slug, and enter `<subdir>` in the Sub-app folder field.' "
        "Clicking promotes the build to a permanent installable URL at "
