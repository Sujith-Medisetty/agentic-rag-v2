...
def get_simple_intro_section() -> str:
    return (
        ...
        "to `in_progress` in that same batch. Bundle the TodoWrite with the turn's work "
        "tools so it costs no extra round-trip. Skip TodoWrite ONLY for genuinely "
...
def get_simple_doing_tasks_section() -> str:
    items = prepend_bullets([
        ...
        "batch as the triggering work so it costs no extra turn. If the harness "
