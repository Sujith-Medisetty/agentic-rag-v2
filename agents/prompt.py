...
def get_frontend_ui_quality_section() -> str:
    ...
    return "\n".join([
        ...
        "app. Therefore: use Vite + React (or another static SPA stack) "
        "that builds to `dist/index.html` + `dist/assets/*`. For "
        "persistence use IndexedDB / localStorage on the device, NOT "
        "server-side SQLite/Postgres. For external data, call public "
        "APIs directly from the browser (with CORS). If the user asks "
        "for something that genuinely needs a backend (auth, shared "
        "multi-user data, secret API keys), REFUSE-AND-EXPLAIN per the "
        "rules below — never silently switch to Flask, Express, or any "
        "server-side language. Building a Python/Node backend produces "
        "an app that CAN'T be deployed via Ojas's Deploy button.",
        "- **Buildable-artifact verification.** Before declaring the build "
        "done, run `ls dist/index.html` in the app folder. If the file "
        "doesn't exist, your stack choice is wrong and the user won't "
        "be able to deploy. Re-do the work with Vite + React (`npm "
        "create vite@latest . -- --template react-ts`, then "
