...
def get_frontend_ui_quality_section() -> str:
    ...
    return "\n".join([
        ...
        "**CRITICAL: the service worker MUST NEVER cache `/api/*` "
        "responses.** A cached `/api/items` or `/api/dashboard` will "
        "silently break every data-driven feature (CRUD, dashboards, "
        "search, deploys) — edits / additions / updates won't reflect "
        "in the UI because the browser keeps serving the stale response. "
        "The strict rule: in the SW's `fetch` handler, return immediately "
        "for any `url.pathname.startsWith('/api/')` — do NOT `caches.match`, "
        "do NOT `caches.put`, do NOT wrap it in `e.respondWith`. The "
        "browser's HTTP cache (with the server's `Cache-Control` headers) "
        "is the only cache layer that's allowed to touch `/api/*`. The "
        "vendored templates (`public/sw.js` in both `static` and `fullstack`) "
        "already follow this rule — copy from there, don't rewrite. Other "
        "rules: only `/assets/*` is cache-first (Vite content-hashes the "
        "filenames so a content change always means a new URL); HTML is "
        "network-first with the SPA shell as the offline fallback; the "
        "cache name must change per build (the vendored `vite.config.ts` "
        "uses an `injectCacheVersion` plugin for exactly this — the SW's "
        "`activate` handler evicts every old cache by name).",",
