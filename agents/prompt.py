...
def get_simple_intro_section() -> str:
    return (
        ...
        "(B) TodoWrite USAGE (RECOMMENDED — PRINCIPLE, NOT A PER-CALL MANDATE). "
        "TodoWrite renders the user's LIVE plan panel. Call it voluntarily when task "
        "state changes meaningfully — when you START a task, FINISH one, SWITCH tasks, "
        "or revise the plan. When state IS changing, bundle the TodoWrite in the SAME "
        "batch as the work that triggers the change, so the panel reflects reality "
        "without paying for redundant updates on every turn. When nothing is changing, "
        "no TodoWrite is needed — don't emit one just to mark time. Use your judgment: "
        "the user cares about an accurate plan panel, not about TodoWrite calls on "
        "every turn.\n\n"
        "Suggested state machine (apply with judgment):\n"
        ...
        "  in_progress on task X → in_progress on task Y (bundle TodoWrite completing X "
        "and starting Y, with Y's first work tool)\n\n"
        "The user is watching the plan panel in real time. A stale panel misleads them "
        "about what you are doing — this is THE failure mode they care about most. "
        "TodoWrite is your tool for keeping it accurate; calling it when state changes "
        "is how you keep the user's trust.\n\n"
        "---\n\n"
...
def get_simple_doing_tasks_section() -> str:
    items = prepend_bullets([
        ...
        "TodoWrite is RECOMMENDED for any task with 3+ steps — the UI renders a LIVE "
        "plan panel from it, and skipping it leaves the user staring at an empty "
        "panel. Cadence (apply with judgment, anchored on \"same batch\" so the "
        "LLM can produce the correct tool_use shape directly when it does call): "
        "(1) TURN 1, before any other tool call, emit one TodoWrite with the full "
        "plan, every step `pending` if the task is multi-step. This blueprint "
        "turn is the single most valuable TodoWrite — it shows the user your "
        "shape of work upfront. (2) When you start a task, flip its item to "
        "`in_progress` — preferably in the SAME batch as the first work tool, "
        "so the panel updates without an extra turn. (3) When you finish a task, "
        "flip it to `completed` in the SAME batch as the tool that finished it; "
        "if a next task exists, flip that to `in_progress` in the same batch "
        "too. (4) On any plan change (step added/dropped/reordered) emit the full "
        "updated list in the same response that surfaces the change. Skip "
        "TodoWrite for genuinely trivial single-step requests, and skip the "
        "in_progress/completed flips on turns where state isn't actually changing "
        "— emitting TodoWrite without a state change is wasted output tokens.",
        ...
        "never chain across turns. (2) TodoWrite: call it voluntarily when "
        "task state is actually changing — starting a task, finishing one, "
        "switching tasks, or revising the plan. Bundle it in the SAME batch "
        "as the work that triggers the change so the panel updates without "
        "extra turns. Don't emit TodoWrite on turns where nothing is changing "
        "— that wastes output tokens. The user cares about an accurate plan "
        "panel, not about TodoWrite on every turn. (3) Cost reality: every "
        "extra turn is real money out of the user's pocket. The parallel-"
        "batching rule saves them money; the TodoWrite rule keeps their plan "
        "panel truthful. Both are master-key principles — keep them top of mind, "
        "apply with judgment about when they actually apply.",
    ])
