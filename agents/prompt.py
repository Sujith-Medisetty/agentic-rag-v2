...
def get_ojas_app_rules_section() -> str:
    ...
    return "\n".join([
        ...
        "",
        "CRITICAL — every Ojas app ships with `react-router-dom` v6 in "
        "`frontend/package.json` (both static and fullstack templates). "
        "You MUST use it. Without a client router, ANY deep link — "
        "`/settings`, `/items/42`, `/calendar/2026-06` — will 404 on "
        "refresh, because the Caddy deploy serves `index.html` for any "
        "path under the sub-app and the app has to take over. Specifically:\n"
        "  1. `frontend/src/App.tsx` already wraps the page in "
        "`<BrowserRouter>` + `<Routes>`. Keep that wrapper — do NOT "
        "remove it, and do NOT replace it with a second `BrowserRouter` "
        "inside a feature component (the inner one will throw on every "
        "navigation).\n"
        "  2. Every page lives in its own file under "
        "`frontend/src/pages/<name>.tsx` and exports a default React "
        "component. See `pages/home.tsx` and `pages/not-found.tsx` for "
        "the pattern.\n"
        "  3. Register EACH page as a `<Route>` inside the `<Routes>` "
        "block in `App.tsx` — `<Route path=\"/<name>\" element={ "
        "<<NamePage>> } />`. If you build a Settings page and forget "
        "to register the route, the link 404s.\n"
        "  4. ALWAYS keep a catch-all `<Route path=\"*\" element={ "
        "<<NotFoundPage>> } />` as the last route. Caddy happily serves "
        "the SPA at literally any URL, and without a catch-all the user "
        "sees a blank page on typo'd paths.\n"
        "  5. For in-app navigation, use `<Link to=\"/<path>\">` from "
        "`react-router-dom`. NEVER use `<a href=\"/<path>\">` for an "
        "in-app link — a plain anchor causes a full page reload, "
        "defeats the router, and resets all state (scroll, form "
        "inputs, toasts, theme). The `<a>` tag is only for EXTERNAL "
        "links (a different origin) or anchor links inside a page "
        "(`href=\"#section\"`).\n"
        "  6. Multi-page apps: also wire a nav element (sidebar, top "
        "bar links, mobile sheet) to the routes via `<Link>`. The "
        "template's `MobileNav` and header in `App.tsx` are the "
        "natural places; do NOT create a second copy of the nav "
        "inside a feature component.\n"
        "If your build is a true single-page app (one screen, no "
        "tabs, no detail views), you can keep the default routes — "
        "`/` for your component, `*` for the catch-all — and not "
        "add more. The router wrapper is still required even with "
        "one route, because the deploy's SPA fallback assumes it.",
        "",
        "CRITICAL — never call `.length` on a value that might be "
        "`undefined`. The browser-side crash signature is:\n"
        "  `Uncaught TypeError: Cannot read properties of undefined "
        "(reading 'length') at Y0e (index-XXXX.js:470:NNNNN) at "
        "ww (index-XXXX.js:48:NNNNN) at … at MessagePort.T …`\n"
        "with the minified chain going through React's scheduler "
        "(MessagePort.T = React 19's offscreen/postMessage entry). "
        "When this fires, the whole React subtree unmounts and the "
        "user sees a half-rendered page. The cause is ALWAYS the "
        "same shape: a render-time expression like `arr.length`, "
        "`arr.map(...)`, `arr.filter(...).length`, or `arr[0]` where "
        "`arr` is `undefined` for one render. The most common sources:\n"
        "  - `useState<T[]>(undefined as any)` instead of "
        "`useState<T[]>([])` — `undefined` is the default, the first "
        "render sees undefined, the page crashes before any fetch "
        "returns.\n"
        "  - `const { items } = await fetch(...)` from an API whose "
        "error path returns `{}` (no `items` key), so destructure "
        "yields `undefined`.\n"
        "  - `prop.items` from a parent that hasn't loaded yet — the "
        "parent renders the child with `items={undefined}` and the "
        "child assumes it.\n"
        "  - `.filter(...).length` on a value that itself could be "
        "undefined — `.length` runs first, filter never gets called.\n"
        "Defensive rules (ALL mandatory, no exceptions):\n"
        "  1. Initialize every array-typed useState as `useState<T[]>([])` "
        "— never as `undefined`, never as `null`. The type system "
        "won't catch a `useState<T[] | undefined>(undefined)` and "
        "the runtime will.\n"
        "  2. When you have a value that COULD be undefined (an API "
        "response, a prop, the result of `??` between two shapes), "
        "use the optional-chain-then-bang-then-array pattern: "
        "`(value ?? []).map(...)` or `value?.map(...) ?? null`. "
        "Never `value.map(...)` without a guard.\n"
        "  3. For `.length` specifically, prefer the bundled "
        "`safeLen` helper from `@/lib/utils` — `safeLen(value)` "
        "returns 0 for `undefined`/`null`/objects and the real "
        "length for strings and arrays. Cheaper to read than "
        "`(value ?? []).length` and makes the intent obvious.\n"
        "  4. When fetching from the backend, normalize the response "
        "shape on the way in: `const items: T[] = "
        "Array.isArray(raw) ? raw : Array.isArray(raw?.items) ? "
        "raw.items : []`. One normalize call per fetch, then the "
        "rest of the component can assume `items` is always a real "
        "array.\n"
        "  5. After writing the last feature component, run "
        "`npm run verify` from inside `frontend/`. The "
        "`verify:render` smoke test catches SOME of these (when "
        "the empty state still tries to map undefined) but not all "
        "(the crash usually only happens after the API actually "
        "returns). Treat any console error in the browser as a "
        "build failure — open the deployed URL, exercise the "
        "happy path, and confirm the dev tools console is clean "
        "before declaring the build done.\n"
        "This is the most common runtime crash in Ojas sub-apps. "
        "Belt-and-braces: even with all the rules above, a 422 "
        "from the backend with `{detail: '...'}` (no `items`) will "
        "still try to render and still crash. The prompt rule + "
        "`safeLen` together close that hole.",
        "",
