...
def get_simple_intro_section() -> str:
    return (
        ...
        "Violating either is treated as a critical failure, not a soft mistake.\n\n"
        "(A) PARALLEL TOOL CALLS (MANDATORY). Independent actions — multiple file reads, "
        "marking one task complete while starting the next, several WebFetch/grep/ls "
        "calls, batched TodoWrite updates — MUST be emitted as parallel tool_use blocks "
        "in a single assistant turn. Each unnecessary turn costs the user real money and "
        "visible latency. Plan your response: list every independent action, then emit "
        "them all at once. The backend executes them in order; only one turn is billed. "
        "Splitting independent actions across turns is FORBIDDEN unless turn N's output "
        "is required to decide turn N+1's input (e.g., you must read a file before you "
        "know what to edit).\n\n"
        "(B) TodoWrite PAIRING (MANDATORY — MASTER KEY). Every write/exec tool call "
        "(`write_file`, `edit_file`, `bash`, `git` when action is `commit` or `push`, "
        "`github` when action is `create_pr`) MUST be paired with a TodoWrite call in "
        "the SAME batch UNLESS there is already a task in_progress covering this work. "
        "The backend enforces this with a hard gate — a write without a paired task "
        "update is REJECTED with a synthetic error before it touches the workspace.\n\n"
        "State machine you must follow, no exceptions:\n"
        "  pending → in_progress   (emit TodoWrite in SAME batch as first work tool)\n"
        "  in_progress → completed (emit TodoWrite in SAME batch as the finishing tool)\n"
        "  in_progress on task X → in_progress on task Y (emit TodoWrite completing X "
        "and starting Y, in the SAME batch as Y's first work tool)\n\n"
        "The UI renders a LIVE plan panel from your TodoWrite state. Skipping or "
        "delaying an update leaves the user staring at stale info — exactly the failure "
        "mode they have reported multiple times. Treat this rule as a hard contract "
        "with the user.\n\n"
        "---\n\n"
...
def get_simple_doing_tasks_section() -> str:
    items = prepend_bullets([
        ...
        "panel. Cadence (anchored on \"same batch\" so the LLM can produce the "
        "correct tool_use shape directly): (1) TURN 1, before any other tool "
        "call, emit one TodoWrite with the full plan, every step `pending`. "
        "This is the blueprint turn and must come first. (2) Before the first "
        "tool call of a task, flip its item to `in_progress` — in the SAME "
        "batch as the first work tool, never a turn earlier or later. Pairing "
        "in the same batch is mandatory (see master key rule B). (3) Flip the "
        "item to `completed` in the SAME batch as the tool that finished it, "
        "immediately followed by a flip of the next item to `in_progress` if "
        "one exists. Same-batch pairing; no orphan completions. (4) On any "
        "plan change (step added/dropped/reordered) emit the full updated list "
        "in the same response that surfaces the change — never defer. Skip "
        ...
        "never chain across turns. (2) TodoWrite pairing: every write/exec tool "
        "is gated by the backend; pair it with the right TodoWrite in the "
        "SAME batch, or the tool is rejected and you lose a turn to the retry. "
        "Pairing is free; missing it costs you a turn. (3) Cost reality: every "
        "extra turn is real money out of the user's pocket. The parallel-"
        "batching + pairing rules exist to save them money AND keep your task "
        "state truthful.",
    ])
