...
class RunnerState(TypedDict, total=False):
    ...
    # estimator, which underestimates by 5-6x and never trips the
    # 80k threshold.
    session_id: str

    # Latest todo list, captured by `node_tools` after a TodoWrite
    # tool call (or initialised to [] on a fresh turn). The
    # `should_continue` end-of-task sync gate reads this to decide
    # whether to nudge the agent into calling TodoWrite one more
    # time before terminating. Storing in state (rather than
    # re-reading the on-disk todo file on every `should_continue`
    # call) is cheaper and keeps the agent-loop single source of
    # truth in `state`. Default reducer = REPLACE; node_tools
    # writes the freshest snapshot.
    last_todos: list[dict]

    # Set to True by `node_force_todo_sync` the first time the
    # end-of-task gate fires. `should_continue` only routes to
    # `node_force_todo_sync` if this is False, so a single nudge
    # per turn is guaranteed. Reset by the next user prompt.
    todo_sync_nudged: bool
