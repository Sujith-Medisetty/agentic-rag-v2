...
# Ported from Rust: runtime/src/usage.rs pricing_for_model()
...
class TokenCounter:
    """
    Tracks cumulative token usage across all turns in a session.
    Ported from Rust: runtime/src/usage.rs UsageTracker
    """
    ...
    def estimate_tokens_in_text(self, text: str) -> int:
        ...
        Mirrors Rust compact.rs estimate_message_tokens: `len / 4 + 1` per block.
        """
        return len(text) // 4 + 1
