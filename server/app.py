...
async def _run_deploy_job(
    *,
    ...
    try:
        # 0. validate
        _set_step(0, "running")
        # NOTE: IncompleteBuild / todo-state guard removed by operator
        # request (2026-06-22). Deploys now proceed regardless of the
        # agent's TodoList state. Other gates (dist quality, build
        # freshness, feature-completeness) still run below.
        ...
        # its id (the short-id convention documented in
        # `_read_session_verify_report`).
        try:
...
# NOTE: IncompleteBuild / todo-state guard removed by operator request
# (2026-06-22). The functions below (_try_self_heal_todos and
# _validate_session_todos) used to be called from the deploy step-0
# entry point. They are no longer wired up, so a deploy will proceed
# regardless of the agent's TodoList state. Other gates (dist
# quality, build freshness, feature-completeness) still run. If you
# want the guard back, re-add the call in step 0 of _run_deploy_job.
...
def _read_session_verify_report(session_id: str) -> dict | None:
    ...
    We pick the first matching project dir under the short-id parent.
